...
class WaveNetBlock(nn.Module):
    ...
    def __init__(self, dilation, num_mels):
        ...
        self.mel_1x1_sigmoid = nn.Conv1d(num_mels, 120, kernel_size=1)
        self.mel_1x1_tanh = nn.Conv1d(num_mels, 120, kernel_size=1)
...
class WaveNet(nn.Module):
    ...
    def __init__(self, num_mels=80, kernel_size=2, residual_channels=120, skip_channels=240,
        ...
        self.upsampling = nn.ConvTranspose1d(num_mels, num_mels, kernel_size=624, stride=64, padding=200,)
        self.blocks = nn.ModuleList()

        for dilation in self.dilations:
            self.blocks.append(WaveNetBlock(dilation, num_mels))
