...
class w2vDiscreteBert(L.LightningModule):
    ...
    def mask_input(self, quantized, mask_prob, mask_span):
        ...
        # For each mask index, compute a span following a normal dist
        mask_spans = torch.zeros_like(mask_start_index).fill_(mask_span).float()
        mask_spans = torch.normal(mean=mask_spans, std=mask_spans)
...
if __name__ == "__main__":
    ...
    vq_w2v_model = w2vDiscreteBert(feat_extractor, context, params=params)
